...
for item in all_inputs:
    if item == user_selected.lower().replace("lighting condition", "lighting").replace("shutter speed", "shutter"): pass # Dont ask for the setting the user is asking to recommend
        ...
        isValid = False
        while not isValid:
            user_in = input(f"Enter your selected setting for {item.title()}: "); user_in = user_in.title().replace("F/" ,"").replace(".0", ""); isValid = any(user_in in d.values() for d in reference_numbers.values())
...
if req_ref > 0 and req_ref <= 8: print(f"Required {user_selected} Value: {reference_numbers[str(req_ref)][user_selected.replace('lighting condition', 'lighting').replace('shutter speed', 'shutter').title()]}")
